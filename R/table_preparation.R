...
#'
#' @param div_code The Tyler code of the division
#' @param .inctable Whether to include table number as prefix; true or false
#' @param .tabnum Number of table in document; E.g., table 12
#'
...
print_div_overall <- function(div_code, .inctable = T, .tabnum = "XX"){
  ...
  glue::glue("Table XX: {rel_div} Division Overall")
...
budget_rename <- function(df_rename){
  ...
  check_type <- function(string){
    dplyr::case_when(string == "perstype" ~ "Expenditure type",
  ...
  df_rename%>%
    ...
    dplyr::rename_with(.fn = ~ tpbudget::year_transform(.x), yrcols) %>%
...
#' @param bud_extract_df A budget extract dataframe to pull year columns from
#'
