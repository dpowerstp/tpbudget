...
#'
#' @examples
oneone_check <- function(df1, df2){
  if (!nrow(df1) == nrow(df2)){

    browser()

    comp <- df2 %>%
      group_by(account.number, accountnum) %>%
      mutate(freq = n())

    stop("Error; number of rows increased merge failure")
  }
...
base_process_extract <- function(df){
  ...
  output <- df %>%
    ...
    dplyr::rename_all(.funs = ~ gsub("-", "_", .x) %>%
                        gsub("#", "num", x = .) %>%
                        gsub("^([0-9])", "yr\\1", .)) %>%
    dplyr::left_join(tpfuncts::tyler_funds, by = c("fund" = "fund_num"))

  # check to confirm one to one merge tyler funds and df
  oneone_check(df, output)
...
read_process_extract <- function(filepath = "./data/Budget Extract Report Custom.xlsx", exp = F){
  ...
  df %>%
    dplyr::mutate(dplyr::across(all_of(yr_rev), .fns = as.numeric)) %>%
...
calc_yearcol <- function(df, curr_fy = 2022){
  ...
  if (any(map_lgl(c(curr_years_adopt, curr_years_proj, act_year_last, next_year), ~ {
    check <- length(.x) != 1
    if (check) print(x)
  ...
  df %>%
    dplyr::mutate(
      ...
      !!dplyr::sym(chng_pct) := !!dplyr::sym(chng_tot) / !!sym(curr_years_proj),
...
budget_list <- function(df, curr_fy = 2022, .missingcols = F){
  ...
  vec_log <- map_lgl(output_dfs, ~ !is.logical(.x))
...
#' @examples{
#'
#' yr_transform("yr2021_2022.total.budget")
#'
#' yr_transform(c("yr2021_2022.ytd.activity.through.april", "yr2021_2022.total.budget"))
#'
#' }
...
gen_dept_summary_rev <- function(dept_code, bud_extract_rev, fy = 2023){
  ...
  yrs <- tobudget::pull_year_cols(bud_extract_rev)

  op_exp <- bud_extract_rev %>%
    dplyr::mutate(across(yrs, ~ -.x)) %>%
...
#'
#' Generates departmental expenses by division dataframe for visualization in tables
#'
#' @param dept_code Department code to generate table for
#' @param bud_dept_list List of budget expenditure dataframes, grouped by different fields
...
#'
#' @examples
gen_dept_summary_div <- function(dept_code, bud_dept_list, fy = 2023){
  yrs_too_early_div <- tpbudget::tpbudget::find_yrstooearly(fy = fy, bud_extract_df = bud_dept_list$department_division, "recent")

  bud_dept_list$department_division %>%
    dplyr::filter(department_num == dept_code) %>%
...
#' @param bud_dept_list List of budget expenditures dataframes grouped in different ways
#' @param fy Fiscal year preparing budget book for
#'
#' @return
#' @export
#'
#' @examples
gen_dept_exp_ov <- function(dept_code, bud_dept_list, fy = 2023){

  yrs_too_early_div <- tpbudget::tpbudget::find_yrstooearly(fy = fy, bud_extract_df = bud_dept_list$fund_department, "recent")


  bud_dept_list$fund_department %>%
    dplyr::filter(department_num == dept_code) %>%
    dplyr::select(-yrs_too_early_div)
}
...
#'
#' @examples
div_overall <- function(divcode, bud_dept_list, fy = 2023){

  yrs_too_early_div <- tpbudget::tpbudget::find_yrstooearly(fy = fy, bud_extract_df = bud_dept_list$fund_department_division, "recent")

  bud_dept_list$fund_department_division %>%
...
#'
#' @examples
gen_dept_summary_perstype <- function(dept_code, bud_dept_list, fy){

  yrs_too_early_deptperstype <- tpbudget::tpbudget::find_yrstooearly(fy = fy, bud_extract_df = bud_dept_list$fund_department_perstype_budget_cat_overall, "recent")

  bud_dept_list$fund_department_perstype_budget_cat_overall %>%
    dplyr::filter(department_num == dept_code) %>%
...
#'
#' @examples
div_perstype <- function(divcode, bud_dept_list, fy = 2023){

  yrs_too_early_div <- tpbudget::tpbudget::find_yrstooearly(fy = fy, bud_extract_df = bud_dept_list$fund_department_division_perstype_budget_cat_overall, "recent")

  df <- bud_dept_list$fund_department_division_perstype_budget_cat_overall %>%
    dplyr::filter(division_num == divcode) %>%
    dplyr::select(-yrs_too_early_div) %>%
    tpbudget::factorize_desc("budget_cat_overall")
