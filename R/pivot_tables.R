...
pt_chng_calc <- function(pt, colname, proj, current, adpt = NULL){
  if (grepl("tot", colname)){
    ...
    #
    pt$defineCalculation(calculationName=colname, calculationExpression = exp, type = "calculation", basedOn = c(adpt, proj), format = "%.0f %%", caption = tpbudget::rename_yeartoyear_depts(colname), noDataCaption = " - ")
...
pt_save <- function(pt, filename, deptadjust = F){
  ...
  saveWorkbook(wb, file = paste0("./data/output/", subdir, "/", filename, ".xlsx"), filename, overwrite = T)
...
#' @param deptadjst Whether you're generating pivot tables to share with departments that include comparisons with the adopted budget book; default F, so no adjustment made
#'
...
pt_rendercreate <- function(df, rowvec = NULL, colvec = NULL, calcrow = F, .ptsavename = NULL, deptadjust = F){
  ...
  pt <- tpbudget::pt_createstandard(df = df, rowvec = rowvec, colvec = colvec, calcrow = calcrow, deptadjust = deptadjst)
