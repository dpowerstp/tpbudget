...
df_propadpt_chng <- function(df_exp, fund_ac = "GF", filternot0 = F, .currfy = 2022){
  ...
  if (!fund_ac %in% uq_funds){
    error(glue("Selected fund type acronym not found in base expenditures dataframe. Fund type must be one of {uq_funds} for this dataframe"))
  }
...
df_all_op_funds <- function(df_rev, df_exp, .fundcol = fund_recode){
  ...
  op_fund_expense <- df_exp %>%
    ...
    dplyr::summarise(across(.cols = yrcols, .fns = ~ round(sum(.x, na.rm = T), digits = 2))) %>%
...
df_fundtable <- function(rev_df, exp_df, fundac, fy = 2023, fundcol = fund_recode, ...){
  ...
  tbudget::df_all_op_funds(df_rev = tpbudget::filter_fund(exp_df, fundac = fundac, ...),
                           df_exp = tpbudget::filter_fund(exp_df, fundac = fundac, ...),
                           .fundcol = {{ fundcol}}) %>%
    ungroup %>%
    select(-yrs_too_early) %>%
